...
# Read Mexico City round 8 snapshot from JSONL
with open('D:/DuMate/Polymarket/WoolBrush-羊毛刷-V2/.dumate/inbox/2026-09-02(1).jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        rec = json.loads(line)
        if rec.get('city') == 'mexico-city' and rec.get('round') == 8 and rec.get('event') == 'city_snapshot':
            snap = rec['data']
            break
...
# Build prompt matching analyze_cmd.rs build_analysis_prompt() + default_llm_prompt()
parts = []

# default_llm_prompt() core
...
parts.append("   - OFFSET 约束（与当地时间联动，三条独立判断，满足任一即 PASS）：偏移量 = (当前档位温度 - YES概率最高的档位温度) / 档位步长。\n")
...
parts.append("## 当前城市数据\n")
parts.append(f"- 城市: Mexico City\n")
parts.append(f"- 时区: America/Mexico_City\n")
parts.append(f"- 当地时间: {local_time}\n")
parts.append(f"- 温度单位: {unit}\n\n")

# Operation bar params
...
parts.append("- OFFSET（与当地时间联动，三条独立判断，满足任一即 PASS）：偏移量 = (当前档位温度 - YES概率最高档位温度) / 档位步长。\n")
parts.append("  - 条件1：当地时间 <= 13 时，偏移量 >= 3 则 PASS\n")
parts.append("  - 条件2：当地时间 > 13 时，偏移量 >= 2 则 PASS\n")
parts.append("  - 条件3：当地时间 > 16 且 ST 实时温度 < ST 最高温度时，偏移量 >= 1 则 PASS\n")
parts.append("  以上三条为独立判断，满足任一即 PASS，全部不满足才 FAIL。\n\n")
...
parts.append(f"- 当天已检测到的最高温度: {awc_max:.1f}{unit}\n")
...
print(f"ST: high={st_high}, current={st_current}")
print()
