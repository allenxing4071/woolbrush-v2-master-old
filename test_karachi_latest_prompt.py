#!/usr/bin/env python
"""
Test: Use the latest build_analysis_prompt() logic from analyze_cmd.rs
to build the prompt for Karachi Round 89, send to Ollama gemma4:31b.

This script faithfully reproduces the Rust code's prompt building,
including default_llm_prompt() and the two-step reasoning output format.
"""

import sqlite3, json, requests, re, sys

# ── 1. Load LLM config from DB ──
db_path = 'D:/DuMate/Polymarket/WoolBrush-羊毛刷-V2/data/woolbrush.db'
conn = sqlite3.connect(db_path)
cur = conn.cursor()
cur.execute('SELECT ollama_api_key, ollama_url, ollama_model, llm_prompt FROM user_settings WHERE id=1')
row = cur.fetchone()
api_key, api_url, model, custom_prompt = row[0], row[1], row[2], row[3]
conn.close()

# ── 2. Load Round 89 snapshot from inbox ──
with open('D:/DuMate/Polymarket/WoolBrush-羊毛刷-V2/.dumate/inbox/2026-09-02(2).jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        rec = json.loads(line)
...
city_tz = snap.get('city_tz', 'Asia/Karachi')
thresholds = snap['thresholds']

# Check held positions (from snapshot positions)
...
# ── 3. Build prompt: faithfully reproduce build_analysis_prompt() ──

# 3a. default_llm_prompt() (when DB llm_prompt is empty)
...
7. 必须遵守操作栏参数约束：
   ...
   - OFFSET 约束（与当地时间联动，三条独立判断，满足任一即 PASS）：偏移量 = (当前档位温度 - YES概率最高档位温度) / 档位步长。
...
prompt.append('- OFFSET（与当地时间联动，三条独立判断，满足任一即 PASS）：偏移量 = (当前档位温度 - YES概率最高档位温度) / 档位步长。\n')
...
# 3h. Threshold table (matches code format!("{:.3}"))
...
# 3i. Two-step reasoning output format (code lines 465-482)
...
prompt.append('- GATE-OFFSET：偏移量 = (档位温度 - YES概率最高档位温度) / 步长 = 具体数值。独立判断以下三条，满足任一即 PASS：\n')
...
# ── 4. Simulate backend hard gate (apply_hard_gate_filter) ──
print("=== Backend Hard Gate (apply_hard_gate_filter) ===")
st_peak_c = st.get('high')
met_peak_c = max(f['temp'] for f in met) if met else None
peak_c = max(st_peak_c, met_peak_c) if st_peak_c and met_peak_c else (st_peak_c or met_peak_c)
print(f"  ST peak: {st_peak_c}°C, MET peak: {met_peak_c}°C, forecast_peak: {peak_c}°C")
print(f"  MARGIN threshold: 3.0°C")

# Find YES-highest threshold (market consensus)
yes_max = max(t['yes_price'] for t in thresholds)
yes_max_labels = [t['label'] for t in thresholds if t['yes_price'] == yes_max]
print(f"  YES-highest: {yes_max_labels[0]} (yes={yes_max:.3f})")

print(f"\n  Thresholds with ask price:")
for t in thresholds:
    if t.get('ask') is not None:
        label = t['label']
        # Parse threshold temp
        m = re.search(r'(\d+(?:\.\d+)?)', label)
        thresh_temp = float(m.group(1)) if m else None
        is_below = 'below' in label.lower()
        if thresh_temp is not None and not is_below:
            margin = thresh_temp - peak_c
            gate = 'PASS' if margin >= 3.0 else 'FAIL'
            print(f"    {label}: temp={thresh_temp}°C, margin={margin:.1f}°C, GATE-MARGIN={gate}")
        elif is_below:
            print(f"    {label}: EXEMPT (or below)")
        else:
            print(f"    {label}: cannot parse temp")
print()
...
try:
    ...
    if resp.status_code == 200:
        ...
        print("=" * 60)
        print("=== LLM RESPONSE (gemma4:31b, latest prompt) ===")
        print("=" * 60)
        print(content)
        print("=" * 60)
        print()

        # Extract all JSON blocks (to check for multiple JSON issue)
        json_blocks = re.findall(r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}', content)
        print(f"Found {len(json_blocks)} JSON block(s) in response")

        # Also try extract_first_json_object (like Rust code does)
        depth = 0
        start = None
        first_json = None
        for i, c in enumerate(content):
            if c == '{':
                ...
                if depth == 0 and start is not None:
                    first_json = content[start:i+1]
                    break

        if first_json:
            print(f"\n=== extract_first_json_object result (what code uses) ===")
            try:
                ...
                print(f"Raw (first 300): {first_json[:300]}")
        else:
            print("No JSON found in response")

        # Show all JSON blocks if multiple
        if len(json_blocks) > 1:
            print(f"\n=== WARNING: {len(json_blocks)} JSON blocks found! ===")
            for i, block in enumerate(json_blocks):
                ...
                try:
                    ...
                    print(f"  (parse failed, first 100 chars: {block[:100]})")
